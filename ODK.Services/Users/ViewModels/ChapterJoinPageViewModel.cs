...
public class ChapterJoinPageViewModel
{
    ...
    /// <summary>
    /// True when the invite names a member who already has an account. Accepting it is then a matter of
    /// signing in and answering the group's questions rather than signing up, so the page asks them to sign in -
    /// a sign-up form could only tell them the address is already taken, and leave the invite outstanding.
    /// </summary>
    public required bool InvitedMemberHasAccount { get; init; }

    /// <summary>
    /// Pre-filled from the invite where there is one, so an invited member sees the details their group
    /// already holds rather than an empty form. Editable: the import supplied them, and they may be wrong. It
    /// also carries the invite token the form posts back.
    /// </summary>
