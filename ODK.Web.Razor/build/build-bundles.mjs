 ...
 *
 * Run with `npm run build:bundles`. The csproj runs it on every build, after the client libraries are
 * restored - some bundles are built from wwwroot/lib, which is itself generated.
 *
 ...
 *
 * The outputs are committed, the same way the compiled CSS is, so a clean checkout serves the app before
 * anything has run; nothing in them should be edited by hand.
 */
...
// Prepended after minifying, so it survives it. These files are committed, so they need to say what they are.
