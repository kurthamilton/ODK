...
- **A string holding HTML is named with an `Html` suffix, everywhere it appears** - the entity property, the
  service view model, the web form view model and the Razor expression (`DescriptionHtml`, `AnswerHtml`,
  `BodyHtml`). `Html.Raw` renders a `*Html` value and nothing else, and a `*Html` value is never rendered
  bare. Both mistakes compile and render - HTML through the encoder prints its own tags, text through
  `Html.Raw` is an injection hole - and a `string` carries no trace of which it holds, so the name is the
  only thing telling them apart. `HtmlRenderingTests` scans the markup for both.
  The suffix is mechanical, including where the name already ends in a content word (`RegisterText` ->
  `RegisterTextHtml`). It marks the value, not what surrounds it: a flag saying whether a group overrides
  its email body is `OverrideBody`, since a bool is not markup.
  Two consequences on a form property. Its label comes from the property name, so a renamed field states the
  old wording in a `[DisplayName]`. And the rename changes the posted field name, which the E2E page objects
  select on from a solution this one cannot see - so grep `E2E/` for the old name.
  **The database columns do not carry the suffix yet**: `Mapping/` pins each renamed property to its existing
  column with `HasColumnName`, and the columns follow in their own expand-then-contract migrations. A pin
  stays until the column it names is gone.
