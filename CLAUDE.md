...
- **A string holding HTML is named with an `Html` suffix, everywhere it appears** - the database column, the
  entity property, the service view model, the web form view model and the Razor expression
  (`DescriptionHtml`, `AnswerHtml`, `BodyHtml`). `Html.Raw` renders a `*Html` value and nothing else, and a
  `*Html` value is never rendered bare. Both mistakes compile and render - HTML through the encoder prints
  its own tags, text through `Html.Raw` is an injection hole - and a `string` carries no trace of which it
  holds, so the name is the only thing telling them apart. `HtmlRenderingTests` scans the markup for both.
